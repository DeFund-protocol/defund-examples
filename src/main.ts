...
const convertTx = await sdk.executeAssetsConvert(
  signer.address,
  fundAddress,
  convertParams
);
...
console.log("fund assets: ", assets);

/*
 * getFundInfo
 */
const lpAddress = "The address of the lp you want to query"; // Optional
const withAssets = true; // false without assets info
const info = await sdk.getFundInfo(fundAddress, lpAddress, withAssets);
